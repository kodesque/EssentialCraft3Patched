...
public class BaublesAccessory extends Item
    ...
    public static String[] names = new String[] { "portableMD", // 0
        "ringOfStability", // 1
        "ringOfExperience", // 2
        "ringOfResistance", // 3
        "lifePendant", // 4
    ...
    public BaubleType[] btALST = new BaubleType[] { BaubleType.AMULET, BaubleType.RING, BaubleType.RING,
        BaubleType.RING, BaubleType.AMULET, BaubleType.AMULET, BaubleType.AMULET, BaubleType.BELT, BaubleType.RING,
        BaubleType.RING, BaubleType.RING, BaubleType.RING, BaubleType.BELT, BaubleType.AMULET, BaubleType.AMULET,
        BaubleType.BELT, BaubleType.RING, BaubleType.RING, BaubleType.RING, BaubleType.RING, BaubleType.AMULET,
        BaubleType.BELT, BaubleType.AMULET, BaubleType.BELT, BaubleType.RING, BaubleType.RING, BaubleType.RING,
        BaubleType.RING, BaubleType.RING, BaubleType.RING, BaubleType.RING, BaubleType.RING, BaubleType.RING, };

    public BaublesAccessory() {
        this.setMaxDamage(0);
    ...
    public void onWornTick(ItemStack itemstack, EntityLivingBase player) {
        if (itemstack.getItemDamage() == 0 && player instanceof EntityPlayer) {
            int ubmrucu = ApiCore.getPlayerData((EntityPlayer) player)
                .getPlayerUBMRU();
            if (ubmrucu - 1 >= 0 && this.setMRU(itemstack, 10)) {
                --ubmrucu;
                ApiCore.getPlayerData((EntityPlayer) player)
                    .modifyUBMRU(ubmrucu);
            }
        }
        if (itemstack.getItemDamage() == 17 && player instanceof EntityPlayer) {
            ...
            if (p.ticksExisted % 10 == 0) ApiCore.getPlayerData((EntityPlayer) player)
                .modifyUBMRU(
                    ApiCore.getPlayerData((EntityPlayer) player)
                        .getPlayerUBMRU() + 1);
        }
        if (itemstack.getItemDamage() == 18 && player instanceof EntityPlayer) {
            ...
            if (p.worldObj.provider != null && p.worldObj.provider.dimensionId != Config.dimensionID)
                RadiationUtils.increasePlayerRadiation(p, -3);
        }
        if (itemstack.getItemDamage() == 19 && player instanceof EntityPlayer) {
            EntityPlayer p = (EntityPlayer) player;
            if (p.worldObj.provider != null && p.worldObj.provider.dimensionId == Config.dimensionID)
                RadiationUtils.increasePlayerRadiation(p, 1);
        }
        if (itemstack.getItemDamage() == 7 && player instanceof EntityPlayer) {
            EntityPlayer p = (EntityPlayer) player;
            PotionEffect[] peArray = new PotionEffect[p.getActivePotionEffects()
                .size()];
            peArray = (PotionEffect[]) p.getActivePotionEffects()
                .toArray(peArray);
    ...
    public static boolean isPotionBad(Potion p) {
        return p == Potion.wither || p == Potion.poison
            || p == Potion.hunger
            || p == Potion.blindness
            || p == Potion.confusion
    ...
    public float getModifiedValue(float original, ItemStack mod, Random rng, EntityPlayer p) {
        ...
        if (mod.getItemDamage() == 20) {
            ...
            List<EntityMob> mobs = p.worldObj.getEntitiesWithinAABB(
                EntityMob.class,
    ...
    public int getMRU(ItemStack stack) {
        return stack.getItemDamage() == 0 ? MiscUtils.getStackTag(stack)
            .getInteger("mru") : 0;
    }
    ...
    public float getModifier(ItemStack stk, EntityPlayer p) {
        if (stk.getItemDamage() == 5) {
            if (!p.worldObj.isRemote && p.worldObj.rand.nextFloat() <= 0.15F) {
                ItemStack windKeeper = new ItemStack(ECItems.windKeeper);
                ...
                p.addChatMessage(
                    new ChatComponentText(
                        EnumChatFormatting.DARK_RED + ""
                            ...
                            + "The wind catcher catches some wind..."));
            }
            return -0.1F;
        }
