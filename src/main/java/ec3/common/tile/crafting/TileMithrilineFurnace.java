...
public class TileMithrilineFurnace extends TileEntity implements IInventory {
    ...
    public void updateEntity() {
        ...
        super.updateEntity();
        if (syncTick == 0) {
            if (this.tracker == null) Notifier.notifyCustomMod(
                "EssentialCraft",
                "[WARNING][SEVERE]TileEntity " + this
                    + " at pos "
                    + this.xCoord
                    + ","
                    + this.yCoord
        ...
        if (correct) {
            Coord3D[] possiblePowerSources = new Coord3D[] { new Coord3D(0, 1, 0), new Coord3D(1, 0, 1),
                new Coord3D(-1, 0, 1), new Coord3D(1, 0, -1), new Coord3D(-1, 0, -1), new Coord3D(2, 1, 0),
                new Coord3D(-2, 1, 0), new Coord3D(0, 1, 2), new Coord3D(0, 1, -2), new Coord3D(2, 2, 2),
                new Coord3D(-2, 2, 2), new Coord3D(2, 2, -2), new Coord3D(-2, 2, -2)

            };
            if (this.energy < maxEnergy) for (int i = 0; i < possiblePowerSources.length; ++i) {
            ...
            if (this.getStackInSlot(0) != null) {
                ...
                if (rec != null && this.getStackInSlot(0).stackSize >= rec.requiredRecipeSize) {
                    ...
                    if (this.getStackInSlot(1) == null || (this.getStackInSlot(1)
                        .isItemEqual(rec.result)
                        && this.getStackInSlot(1).stackSize + rec.result.stackSize <= this.getStackInSlot(1)
                            .getMaxStackSize())) {
            ...
            for (int i = 0; i < 10; ++i) EssentialCraftCore.proxy.FlameFX(
                xCoord + 0.5F + MathUtils.randomFloat(this.worldObj.rand) * 0.4F,
                yCoord + 0.2F + MathUtils.randomFloat(this.worldObj.rand) * 0.6F,
                zCoord + 0.5F + MathUtils.randomFloat(this.worldObj.rand) * 0.4F,
                0,
                0.01F,
                0,
                0D,
    ...
    public boolean isStructureCorrect() {
        ...
        int y = this.yCoord;
        int z = this.zCoord;
        boolean hasPlatformBelow = worldObj.getBlock(x, y - 1, z) == ECBlocks.invertedBlock
            && worldObj.getBlock(x - 1, y - 1, z) == ECBlocks.invertedBlock
            && worldObj.getBlock(x + 1, y - 1, z) == ECBlocks.invertedBlock
            && worldObj.getBlock(x, y - 1, z - 1) == ECBlocks.invertedBlock
            && worldObj.getBlock(x, y - 1, z + 1) == ECBlocks.invertedBlock
            && worldObj.getBlock(x + 1, y - 1, z + 1) == ECBlocks.invertedBlock
            && worldObj.getBlock(x + 1, y - 1, z - 1) == ECBlocks.invertedBlock
            && worldObj.getBlock(x - 1, y - 1, z + 1) == ECBlocks.invertedBlock
            && worldObj.getBlock(x - 1, y - 1, z - 1) == ECBlocks.invertedBlock
            && worldObj.getBlock(x - 2, y - 1, z) == ECBlocks.invertedBlock
            && worldObj.getBlock(x + 2, y - 1, z) == ECBlocks.invertedBlock
            && worldObj.getBlock(x, y - 1, z - 2) == ECBlocks.invertedBlock
            && worldObj.getBlock(x, y - 1, z + 2) == ECBlocks.invertedBlock;

        boolean hasGenericOutline = worldObj.getBlock(x - 2, y, z) == ECBlocks.invertedBlock
            && worldObj.getBlock(x + 2, y, z) == ECBlocks.invertedBlock
            && worldObj.getBlock(x, y, z - 2) == ECBlocks.invertedBlock
            && worldObj.getBlock(x, y, z + 2) == ECBlocks.invertedBlock
            && worldObj.getBlock(x - 2, y, z + 2) == ECBlocks.invertedBlock
            && worldObj.getBlock(x + 2, y, z - 2) == ECBlocks.invertedBlock
            && worldObj.getBlock(x - 2, y, z - 2) == ECBlocks.invertedBlock
            && worldObj.getBlock(x + 2, y, z + 2) == ECBlocks.invertedBlock
            && worldObj.getBlock(x - 2, y + 1, z + 2) == ECBlocks.invertedBlock
            && worldObj.getBlock(x + 2, y + 1, z - 2) == ECBlocks.invertedBlock
            && worldObj.getBlock(x - 2, y + 1, z - 2) == ECBlocks.invertedBlock
            && worldObj.getBlock(x + 2, y + 1, z + 2) == ECBlocks.invertedBlock;
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "MithrilineFurnaceSettings",
                "tileentities",
                new String[] { "Maximum enderstar pulse stored:10000" },
                "");
            String dataString = "";
