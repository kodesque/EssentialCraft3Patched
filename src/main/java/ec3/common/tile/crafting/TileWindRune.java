...
public class TileWindRune extends TileEntity {
    ...
    public int getEnderstarEnergy() {
        ...
        int[][] coords = new int[][] { { 2, 0, 2 }, { 2, 0, -2 }, { -2, 0, 2 }, { -2, 0, -2 }, { 1, 2, 1 },
            { -1, 2, 1 }, { 1, 2, -1 }, { -1, 2, -1 } };
        int energy = 0;

        for (int i = 0; i < 8; ++i) {
            TileEntity tile = this.worldObj
                .getTileEntity(xCoord + coords[i][0], yCoord + coords[i][1], zCoord + coords[i][2]);
                ...
                if (hasEnergy || creative) {
                    ...
                    for (int i = 0; i < 8; ++i) {
                        ...
                        if (tile instanceof TileMithrilineCrystal) {
                            ...
                            if (hasEnergy) TileMithrilineCrystal.class.cast(tile).energy = 0;

                            if (cenergy >= energy) {
                                int left = cenergy - energy;
                                if (hasEnergy) TileMithrilineCrystal.class.cast(tile).energy = left;

                                if (rec.transforming.getItem() instanceof ItemSoulStone && !ECUtils.getData(player)
                                    .isWindbound()) {
                                    ECUtils.getData(player)
                                        .modifyWindbound(true);
                                    if (!player.worldObj.isRemote) {
                                        player.addChatMessage(
                                            new ChatComponentText(StatCollector.translateToLocal("ec3.txt.windImbue"))
                                ...
                                if (player.inventory.getStackInSlot(player.inventory.currentItem) == null)
                                    player.inventory
                                        .setInventorySlotContents(player.inventory.currentItem, rec.result.copy());
        ...
        if (tier >= 0) {
            ...
            for (int i = 0; i < 8; ++i) {
                this.worldObj.spawnParticle(
                    "reddust",
                    xCoord + coords[i][0] + 0.5D,
                    yCoord + coords[i][1] + movement / 30,
                    zCoord + coords[i][2] + 0.5D,
                    -1,
                    1,
                    0);
                this.worldObj.spawnParticle(
                    "reddust",
                    xCoord + coords[i][0] + 0.5D,
                    yCoord + coords[i][1] + 2 + movement / 30,
                    zCoord + coords[i][2] + 0.5D,
                    -1,
                    1,
                    0);
    ...
    public void checkStructureAndTier() {
        ...
        int z = this.zCoord;
        tier = -1;
        if (w.getBlock(x, y, z) == ECBlocks.invertedBlock && w.getBlock(x + 1, y, z) == ECBlocks.invertedBlock
            && w.getBlock(x - 1, y, z) == ECBlocks.invertedBlock
            && w.getBlock(x, y, z + 1) == ECBlocks.invertedBlock
            && w.getBlock(x, y, z - 1) == ECBlocks.invertedBlock
            && w.getBlock(x + 1, y + 1, z - 1) == ECBlocks.invertedBlock
            && w.getBlock(x + 1, y + 1, z + 1) == ECBlocks.invertedBlock
            && w.getBlock(x - 1, y + 1, z - 1) == ECBlocks.invertedBlock
            && w.getBlock(x - 1, y + 1, z + 1) == ECBlocks.invertedBlock
            && w.getBlock(x + 1, y + 2, z - 1) == ECBlocks.invertedBlock
            && w.getBlock(x + 1, y + 2, z + 1) == ECBlocks.invertedBlock
            && w.getBlock(x - 1, y + 2, z - 1) == ECBlocks.invertedBlock
            && w.getBlock(x - 1, y + 2, z + 1) == ECBlocks.invertedBlock
            && w.getBlock(x - 1, y + 3, z + 1) == ECBlocks.mithrilineCrystal
            && w.getBlock(x + 1, y + 3, z + 1) == ECBlocks.mithrilineCrystal
            && w.getBlock(x - 1, y + 3, z - 1) == ECBlocks.mithrilineCrystal
            && w.getBlock(x + 1, y + 3, z - 1) == ECBlocks.mithrilineCrystal
            && w.getBlock(x - 2, y + 1, z + 2) == ECBlocks.mithrilineCrystal
            && w.getBlock(x + 2, y + 1, z + 2) == ECBlocks.mithrilineCrystal
            && w.getBlock(x - 2, y + 1, z - 2) == ECBlocks.mithrilineCrystal
            && w.getBlock(x + 2, y + 1, z - 2) == ECBlocks.mithrilineCrystal

            && w.getBlock(x + 1, y, z - 1) == ECBlocks.platingPale
            && w.getBlock(x + 2, y, z - 1) == ECBlocks.platingPale
            && w.getBlock(x - 2, y, z - 1) == ECBlocks.platingPale
            && w.getBlock(x + 2, y, z + 1) == ECBlocks.platingPale
            && w.getBlock(x - 2, y, z + 1) == ECBlocks.platingPale
            && w.getBlock(x + 1, y, z - 2) == ECBlocks.platingPale
            && w.getBlock(x - 1, y, z - 2) == ECBlocks.platingPale
            && w.getBlock(x + 1, y, z + 2) == ECBlocks.platingPale
            && w.getBlock(x - 1, y, z + 2) == ECBlocks.platingPale

            && w.getBlock(x - 2, y, z) == ECBlocks.magicPlating
            && w.getBlock(x - 2, y + 1, z) == ECBlocks.magicPlating
            && w.getBlock(x + 2, y, z) == ECBlocks.magicPlating
            && w.getBlock(x + 2, y + 1, z) == ECBlocks.magicPlating
            && w.getBlock(x, y, z - 2) == ECBlocks.magicPlating
            && w.getBlock(x, y + 1, z - 2) == ECBlocks.magicPlating
            && w.getBlock(x, y, z + 2) == ECBlocks.magicPlating
            && w.getBlock(x, y + 1, z + 2) == ECBlocks.magicPlating
