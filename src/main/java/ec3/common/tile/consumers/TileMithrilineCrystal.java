...
public class TileMithrilineCrystal extends TileEntity {
    ...
    public void updateEntity() {
        ...
        if (hasSky) {
            float energyGenerated = this.worldObj.provider != null && this.worldObj.provider.dimensionId == 1
                ? energyEachTick_End
                : energyEachTick;
            if (energy < maxEnergy) energy += energyGenerated;
        }
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "MithrilineCrystalSettings",
                "tileentities",
                new String[] { "Enderstar pulse generated each tick:0.025",
                    "Enderstar pulse generated each tick in the end:0.1", "Requires a direct view to the sky:true",
