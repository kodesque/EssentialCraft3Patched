...
public class TileMRUCoil extends TileMRUGeneric {
            ...
            if (this.worldObj.isRemote) {
                if (this.isStructureCorrect()) {
                    if (this.localLightning == null) {
                        {
                            this.localLightning = new ArcFX(
                                this.worldObj.rand,
                                new Coord3D(0.5F, 0.9F, 0.5F),
                                new Coord3D(
                                    0.5F + MathUtils.randomDouble(this.worldObj.rand),
                                    0.9F + MathUtils.randomDouble(this.worldObj.rand),
                                    0.5F + MathUtils.randomDouble(this.worldObj.rand)),
                                0.1F,
                                1.0F,
                                0.2F,
                        ...
                        if (this.localLightning.renderTicksExisted >= 20) this.localLightning = null;
                    }
                } else this.localLightning = null;
            }
            if (this.monsterLightning != null) {
                if (!this.worldObj.isRemote) ++lightningTicks;
            ...
            if (this.isStructureCorrect) {
                List<EntityLivingBase> entities = this.worldObj.getEntitiesWithinAABB(
                    ...
                    AxisAlignedBB.getBoundingBox(xCoord, yCoord, zCoord, xCoord + 1, yCoord + 1, zCoord + 1)
                        .expand(rad * radiusModifier, rad * radiusModifier, rad * radiusModifier));
                        ...
                        if (b instanceof EntityPlayer) {
                            if (!((EntityPlayer) b).capabilities.isCreativeMode && hurtPlayers) {
                                ...
                                if (is != null && is.getItem() instanceof ItemPlayerList) {
                                    ...
                                    if (!itemTag.hasKey("usernames")) itemTag.setString("usernames", "||username:null");
                            ...
                            if (!(b instanceof IMob) && !hurtPassive) continue Ford;
                            if (b instanceof IMRUPresence) continue Ford;
                            attack(b);
                            break Ford;
                        }
                    }
    ...
    public void attack(EntityLivingBase b) {
        if (this.getMRU() > mruUsage && !b.isDead && b.hurtTime <= 0 && b.hurtResistantTime <= 0) {
            if (generatesCorruption) ECUtils
                .increaseCorruptionAt(worldObj, xCoord, yCoord, zCoord, this.worldObj.rand.nextInt(genCorruption));
            b.attackEntityFrom(DamageSource.magic, damage);
            if (this.worldObj.isRemote && this.monsterLightning == null) this.worldObj.playSound(
                xCoord + 0.5F,
                yCoord + 0.5F,
                zCoord + 0.5F,
                ...
                2F,
                false);
            this.monsterLightning = new ArcFX(
                this.worldObj.rand,
                new Coord3D(0.5F, 0.8F, 0.5F),
                new Coord3D(b.posX - this.xCoord + 0.5D, b.posY - this.yCoord + 0.8D, b.posZ - this.zCoord + 0.5D),
                0.1F,
                1F,
                0.0F,
                0.7F);
            if (!this.worldObj.isRemote) this.setMRU(this.getMRU() - mruUsage);
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        AxisAlignedBB bb = super.getRenderBoundingBox().expand(1, 1, 1);
        return bb;
    }
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "MRUCoilSettings",
                "tileentities",
                new String[] { "Max MRU:" + ApiCore.DEVICE_MAX_MRU_GENERIC * 10, "MRU Usage per hit:1000",
                    ...
                    "Can the coil attack passive entities, like sheep:true", "Damage per hit:18.0",
