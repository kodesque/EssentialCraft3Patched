...
import ec3.utils.dummycore.utils.BlockPos;
...
public class TileChamberController extends TileEntity implements ITEHasMRU {
    ...
    public List<BlockPos> blocksInStructure = new ArrayList<BlockPos>();
    ...
    public void updateEntity() {
        ...
        } else--this.structureCheckTick;

        // Sending the sync packets to the CLIENT.
        if (syncTick == 0) {
            if (!this.worldObj.isRemote) MiscUtils.sendPacketToAllAround(
                worldObj,
    ...
    public IMRUPresence getMRUCU() {
        if (isCorrect) {
            List<IMRUPresence> pList = this.worldObj.getEntitiesWithinAABB(
                IMRUPresence.class,
                AxisAlignedBB.getBoundingBox(
                    ...
                    upperCoord.y,
                    upperCoord.z));
            if (pList != null && !pList.isEmpty()) return pList.get(0);
        }
        return null;
    }
    ...
    public boolean checkStructure() {
        ...
        int maxZ = 0;
        int checkInt0 = 0;
        List<?> allowedBlocks = ECUtils.allowedBlocks.get(EnumStructureType.MRUCUContaigementChamber); // Getting the
                                                                                                       // list of
                                                                                                       // allowed blocks
                                                                                                       // in the
                                                                                                       // structure
                            ...
                            if (allowedBlocks.contains(this.worldObj.getBlock(xCoord + x, yCoord + y, zCoord + z))) {
                                ...
                                if (ECUtils.ignoreMeta.containsKey(
                                    this.worldObj.getBlock(xCoord + x, yCoord + y, zCoord + z)
                                        .getUnlocalizedName())
                                    && ECUtils.ignoreMeta.get(
                                        this.worldObj.getBlock(xCoord + x, yCoord + y, zCoord + z)
                                            .getUnlocalizedName())) {
                                    meta = -1;
                                }
                                DummyData dt = new DummyData(
                                    this.worldObj.getBlock(xCoord + x, yCoord + y, zCoord + z)
                                        .getUnlocalizedName(),
                                    meta);
                                ...
                                dt = null;
                                if (this.worldObj.getTileEntity(xCoord + x, yCoord + y, zCoord + z) != null
                                    && this.worldObj
                                        .getTileEntity(xCoord + x, yCoord + y, zCoord + z) instanceof IStructurePiece) {
                                    IStructurePiece piece = (IStructurePiece) this.worldObj
                                    ...
                                    if (this.worldObj.getTileEntity(
                                        xCoord + x,
                                        yCoord + y,
                                        zCoord + z) instanceof TileHoldingChamber) {
    ...
    @Override
    public int getMRU() {
        IMRUPresence pressence = getMRUCU();
        if (pressence != null) return this.mru;
        return 0;
    }
    ...
    public float getBalance() {
        IMRUPresence pressence = getMRUCU();
        if (pressence != null) return pressence.getBalance();
        return this.balance;
    }
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "EnrichmentChamberSettings",
                "tileentities",
                new String[] { "Default Max MRU:60000", "MRU Increasement per Storage:100000" },
                "");
            String dataString = "";
