package ec3.common.tile.transport;
...
public class TileMagicalMirror extends TileMRUGeneric {
            ...
            if (tile != null && tile instanceof ISidedInventory && tile instanceof IInventory) {
                ...
                f: for (int i : slots) {
                    ...
                    if (is != null) {
                        ...
                        if (slot != -1) {
                            if (this.transferingStack == null) this.transferingStack = is;
                            if (this.transferTime < 60) ++this.transferTime;
                            ...
                            if (this.worldObj.getWorldTime() % 5 == 0) ECUtils.spawnItemFX(sX, sY, sZ, dX, dY, dZ);
        ...
        for (int i = 2; i < 17; ++i) {
            ...
            if (stk == null) {
                for (int j = 0; j < lst.size(); ++j) {
                    UnformedItemStack uis = lst.get(j);
                    if (uis != null && uis.itemStackMatches(is)) return i;
                }
            }
        }
        return -1;
    ...
    public void updateEntity() {
        if (this.worldObj.isRemote) {
            if (this.pulsing && this.transferTime < 60) ++this.transferTime;
        }
        super.updateEntity();
        if (this.inventoryPos != null) {
            TileEntity tile = this.worldObj.getTileEntity(
                MathHelper.floor_double(this.inventoryPos.x),
                MathHelper.floor_double(this.inventoryPos.y),
                MathHelper.floor_double(this.inventoryPos.z));
        ...
        if (tile != null && tile instanceof TileMagicalAssembler && this.inventoryPos != null) {
            ...
            TileEntity tile1 = this.worldObj.getTileEntity(
                MathHelper.floor_double(this.inventoryPos.x),
                MathHelper.floor_double(this.inventoryPos.y),
                MathHelper.floor_double(this.inventoryPos.z));
                ...
                if (assembler.getStackInSlot(17) != null) {
                    ...
                    fori: for (int i = 0; i < inv.getSizeInventory(); ++i) {
                        ...
                        if (is1 == null || (is1.isItemEqual(is) && is1.stackSize + 1 < is1.getMaxStackSize() + 1)) {
                            this.pulsing = true;
                            if (!this.worldObj.isRemote && this.transferTime <= 60) ++this.transferTime;
                            ...
                            if (this.transferTime >= 60) {
                                ItemStack set = is.copy();
                                set.stackSize = 1;
                                if (inv.getStackInSlot(i) == null) inv.setInventorySlotContents(i, set);
                                else++inv.getStackInSlot(i).stackSize;
            ...
            if (tile instanceof IInventory) {
                ...
                if (this.inventoryPos != null) {
                    TileEntity tile1 = this.worldObj.getTileEntity(
                        ...
                        MathHelper.floor_double(this.inventoryPos.y),
                        MathHelper.floor_double(this.inventoryPos.z));
                    if (assembler != null && tile1 != null
                        && tile1 instanceof IInventory
                        && !(tile1 instanceof TileMagicalAssembler)) {
                        IInventory inv = (IInventory) tile1;
                        CycleF: for (int w = 0; w < assembler.getSizeInventory(); ++w) {
                            if (assembler.getStackInSlot(w) != null) {
                                ...
                                for (int i = 0; i < inv.getSizeInventory(); ++i) {
                                    ItemStack is1 = inv.getStackInSlot(i);
                                    if (is1 == null
                                        || (is1.isItemEqual(is) && is1.stackSize + 1 < is1.getMaxStackSize() + 1)) {
                                        this.pulsing = true;
                                        this.syncTick = 0;
                                        if (!this.worldObj.isRemote && this.transferTime <= 60) ++this.transferTime;
    ...
    public void readFromNBT(NBTTagCompound i) {
        if (i.hasKey("coord")) {
            ...
            inventoryPos = new Coord3D(
                Double.parseDouble(coordData[0].fieldValue),
                Double.parseDouble(coordData[1].fieldValue),
                Double.parseDouble(coordData[2].fieldValue));
        } else this.inventoryPos = null;
        ...
        if (i.hasKey("transferingStack")) {
            ...
            if (FMLCommonHandler.instance()
                .getEffectiveSide() == Side.CLIENT) is.stackSize = 1;
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "MagicalMirrorSettings",
                "tileentities",
                new String[] { "Max MRU:" + ApiCore.DEVICE_MAX_MRU_GENERIC, "Max range of item transfering:8.0" },
                "");
            String dataString = "";
