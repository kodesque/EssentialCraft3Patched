...
public class Config implements IDummyConfig {
    ...
    public void loadGUIs() {
        guiID[0] = config.get("GUI", "Generic GUI ID", 7321)
            .getInt();
        guiID[1] = config.get("GUI", "Demon GUI ID", 7322)
            .getInt();
    }
    ...
    public void loadMobs() {
        autoFindEID = config
            .getBoolean("autoFindEntityIDs", "Entities", false, "Enable if you can't set the entity ID's on your own.");
        mobID[0] = ECEntities.nextEntityID(
            config.get("Entities", "MRUPresenceID", 71)
                .getInt());
        mobID[1] = ECEntities.nextEntityID(
            config.get("Entities", "MRUArrowID", 72)
                .getInt());
        mobID[2] = ECEntities.nextEntityID(
            config.get("Entities", "SolarBeamID", 73)
                .getInt());
        mobID[3] = ECEntities.nextEntityID(
            config.get("Entities", "WindMageID", 74)
                .getInt());
        mobID[4] = ECEntities.nextEntityID(
            config.get("Entities", "PosionFumeID", 75)
                .getInt());
    }

    public void loadMisc() {
        biomeID[0] = config.get("Biomes", "ChaosCorruptionID", 91)
            .getInt();
        biomeID[1] = config.get("Biomes", "FrozenCorruptionID", 92)
            .getInt();
        biomeID[2] = config.get("Biomes", "ShadowCorruptionID", 93)
            .getInt();
        biomeID[3] = config.get("Biomes", "MagicCorruptionID", 94)
            .getInt();
        biomeID[4] = config.get("Biomes", "DesertID", 95)
            .getInt();
        biomeID[5] = config.get("Biomes", "DreadLandsID", 96)
            .getInt();
        ...
        String[] cfgCustomOreParsing = config.getStringList(
            "CustomMagmaticAlloys",
            "Misc",
            new String[] { "oreSaltpeter:10065301|5?dustSaltpeter", "oreSulfur:16777113|5?dustSulfur" },
        ...
        for (String s : cfgCustomOreParsing) {
            ...
            EnumHelper.addEnum(
                EnumOreColoring.class,
                oredOreName.toUpperCase(),
                new Class[] { String.class, String.class, int.class, int.class },
                new Object[] { oredOreName, oredResultName, oreColor, oreOutput });
        }
        oreGenAttempts = config.getInt(
            "oreGenAttempts",
            "Misc",
            4,
            0,
        ...
        eMRUCUGenAttempts = config.getInt(
            "eMRUCUGenAttempts",
            "Misc",
            1,
            0,
        ...
        allowHologramInOtherDimensions = config.getBoolean(
            "allowHologramInOtherDimensions",
            "Mobs",
            false,
            "Is the hologram boss allowed to spawn in the overworld/nether/anything but Hoanna");
    }
    ...
    public void loadTiles() {
        ...
        for (Class<? extends TileEntity> tile : ECTiles.cfgDependant) {
            try {
