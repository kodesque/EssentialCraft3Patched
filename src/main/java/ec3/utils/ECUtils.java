...
import ec3.api.items.IItemAllowsSeeingMRUCU;
...
import ec3.api.recipes.ShapedFurnaceRecipe;
...
import ec3.utils.server.ScheduledServerAction;
import ec3.utils.server.ServerToClientSyncAction;
...
public class ECUtils {
    ...
    public static void changePlayerPositionOnClient(EntityPlayer e) {
        if (!e.worldObj.isRemote) {
            ...
            DummyPacket pkt = new DummyPacket(
                "||mod:ec3.player.position" + aaa + "" + aab + "" + aac + "" + aad + "" + aae);
            DummyPacketHandler.sendToPlayer(pkt, (EntityPlayerMP) e);
        }
    }

    public static void playSoundToAllNearby(double x, double y, double z, String sound, float volume, float pitch,
        double radius, int dim) {
        ...
        DummyPacket pkt = new DummyPacket(
            "||mod:ec3.sound" + aaa + "" + aab + "" + aac + "" + aad + "" + aae + "" + aaf);
    ...
    public static void readOrCreatePlayerData(EntityPlayer e, NBTTagCompound tag) {
        ...
        playerDataTable.get(e.getCommandSenderName())
            .readFromNBTTagCompound(tag);
    }
    ...
    public static void initMRUTag(ItemStack stack, int maxMRU) {
        if (maxMRU == 0) {
            try {
                Class<? extends Item> itemClz = stack.getItem()
                    .getClass();
        ...
        if (!getStackTag(stack).hasKey("mru")) getStackTag(stack).setInteger("mru", 0);
    }
    ...
    public static void mruIn(TileEntity tile, int slotNum) {
        if (tile instanceof IInventory && tile instanceof ITEHasMRU) {
            ...
            if (inv.getStackInSlot(slotNum) != null && inv.getStackInSlot(slotNum)
                .getItem() instanceof ItemBoundGem
                && inv.getStackInSlot(slotNum)
                    .getTagCompound() != null) {
                ItemStack s = inv.getStackInSlot(slotNum);
                int[] o = ItemBoundGem.getCoords(s);
                if (MathUtils.getDifference(tile.xCoord, o[0]) <= 16 && MathUtils.getDifference(tile.yCoord, o[1]) <= 16
                    && MathUtils.getDifference(tile.zCoord, o[2]) <= 16) {
                    if (tile.getWorldObj()
                        .getTileEntity(o[0], o[1], o[2]) != null
                        && tile.getWorldObj()
                            .getTileEntity(o[0], o[1], o[2]) instanceof ITEHasMRU) {
                        ITEHasMRU t = (ITEHasMRU) tile.getWorldObj()
                            .getTileEntity(o[0], o[1], o[2]);
            ...
            if (mrut.getMRU() < 0) mrut.setMRU(0);
        }
    }
    ...
    public static boolean increaseCorruptionAt(World w, float x, float y, float z, int amount) {
        ...
        try {
            ...
            List<EntityPlayer> players = w.getEntitiesWithinAABB(
                EntityPlayer.class,
                AxisAlignedBB.getBoundingBox(x, y, z, x + 1, y + 1, z + 1)
                    .expand(6D, 3D, 6D));
            for (int i = 0; i < players.size(); ++i) {
                if (!w.isRemote) calculateAndAddMRUCorruptionPE(players.get(i));
            }
        } catch (Exception e) {
            return false;
        }
    ...
    public static void calculateAndAddMRUCorruptionPE(EntityPlayer player) {
        boolean hasEffect = player.getActivePotionEffect(ECPotions.mruCorruptionPotion) != null;
        if (hasEffect) {
            int currentDuration = player.getActivePotionEffect(ECPotions.mruCorruptionPotion)
                .getDuration();
            int newDuration = currentDuration + 2;
            int newModifier = currentDuration / 2000;
            player.removePotionEffect(ECPotions.mruCorruptionPotion.id);
            player.addPotionEffect(new PotionEffect(ECPotions.mruCorruptionPotion.id, newDuration, newModifier));
        } else {
            player.addPotionEffect(new PotionEffect(ECPotions.mruCorruptionPotion.id, 200, 0));
        }
    }
    ...
    public static void calculateAndAddPE(EntityPlayer player, Potion potion, int index, int index2) {
        ...
        if (hasEffect) {
            int currentDuration = player.getActivePotionEffect(potion)
                .getDuration();
            ...
            if (!(l.get(0) instanceof EntityMRUPresence)) {
                ...
                for (int i = 0; i < actualList.size(); ++i) {
                    if (actualList.get(i) instanceof EntityMRUPresence) {
                        ...
                        Coord3D current = new Coord3D(pressence.posX, pressence.posY, pressence.posZ);
            ...
            if (inv.getStackInSlot(slotNum) != null && inv.getStackInSlot(slotNum)
                ...
                if (MathUtils.getDifference(tile.xCoord, o[0]) <= 16 && MathUtils.getDifference(tile.yCoord, o[1]) <= 16
                    ...
                    if (tile.getWorldObj()
                        ...
                        && tile.getWorldObj()
                            .getTileEntity(o[0], o[1], o[2]) instanceof ITEHasMRU) {
                        float balance = ((ITEHasMRU) tile.getWorldObj()
                            .getTileEntity(o[0], o[1], o[2])).getBalance();
                        ...
                        if (tile instanceof TileRayTower) {
                            if (tile.getWorldObj()
                                .getTileEntity(o[0], o[1], o[2]) instanceof TileRayTower)
                                EssentialCraftCore.proxy.MRUFX(
                                    (float) (o[0] + 0.5D),
                                    (float) (o[1] + 1.85D),
                                    (float) (o[2] + 0.5D),
                                    tile.xCoord - o[0],
                                    tile.yCoord - o[1] + 0.25D,
                                    tile.zCoord - o[2],
                                    colorRRender,
                                    colorGRender,
                                    colorBRender);
                            else if (tile.getWorldObj()
                                .getTileEntity(o[0], o[1], o[2]) instanceof TileMRUReactor)
                                EssentialCraftCore.proxy.MRUFX(
                                    (float) (o[0] + 0.5D),
                                    (float) (o[1] + 1.1D),
                                    (float) (o[2] + 0.5D),
                                    tile.xCoord - o[0],
                                    tile.yCoord - o[1] + 0.8D,
                                    tile.zCoord - o[2],
                                    colorRRender,
                                    colorGRender,
                                    colorBRender);
                            else EssentialCraftCore.proxy.MRUFX(
                                (float) (o[0] + 0.5D),
                                (float) (o[1] + 0.5D),
                                (float) (o[2] + 0.5D),
                                tile.xCoord - o[0],
                                tile.yCoord - o[1] + 1.5D,
                                tile.zCoord - o[2],
                                colorRRender,
                                colorGRender,
                                colorBRender);
    ...
    public static boolean hasActiveEvent() {
        return !ec3WorldTag.hasNoTags() && ec3WorldTag.getString("currentEvent") != null
            && !ec3WorldTag.getString("currentEvent")
                .isEmpty();
    }
    ...
    public static IRecipe findRecipeByIS(ItemStack searched, int recipeType) {
        if (recipeType == 0 || recipeType == 1) {
            List<IRecipe> rec = CraftingManager.getInstance()
                .getRecipeList();
                ...
                if (recipe instanceof ShapedRecipes) {
                    ...
                    if (ItemStack.areItemStackTagsEqual(output, searched) && output.isItemEqual(searched))
                        return new ShapedRecipes(
                            mRecipe.recipeWidth,
                            mRecipe.recipeHeight,
                            mRecipe.recipeItems,
                            mRecipe.getRecipeOutput());
                }
        ...
        if (recipeType == 2 || recipeType == 3) {
            ...
            for (int i = 0; i < rec.size(); ++i) {
                ...
                if (recipe instanceof ShapelessOreRecipe) {
                    ...
                    ItemStack output = mRecipe.getRecipeOutput();
                    if (output.isItemEqual(searched)) return copyShapelessOreRecipe(mRecipe);
                }
            }
            // A compatibility patch
            return !MagicalAssemblerRecipes.findRecipes(searched)
                .isEmpty() ? MagicalAssemblerRecipes.findRecipes(searched)
                    .get(0) : null;
        }
        if (recipeType == 4) {
            Set<?> $s = FurnaceRecipes.smelting()
                .getSmeltingList()
                .entrySet();
    ...
    public static ShapedOreRecipe copyShapedOreRecipe(ShapedOreRecipe recipe) {
        ShapedOreRecipe ret = new ShapedOreRecipe(
            recipe.getRecipeOutput(),
            new Object[] { "ooo", "ooo", "ooo", 'o', Items.stick });
        try {
    ...
    public static boolean oreDictionaryCompare(ItemStack stk, ItemStack stk1) {
        if (stk == null || stk1 == null) return false;

        if (OreDictionary.getOreIDs(stk) == null || OreDictionary.getOreIDs(stk).length == 0
            || OreDictionary.getOreIDs(stk1) == null
            || OreDictionary.getOreIDs(stk1).length == 0) return false;
    ...
    public static boolean canFilterAcceptItem(IInventory filterInventory, ItemStack is, ItemStack filter) {
        if (filter.getItemDamage() == 0 || filter.getItemDamage() == 2) {
                ...
                if (f != null) {
                    if (f.getItem() instanceof ItemFilter) {
                        ...
                        if (filter.getItemDamage() == 2) {
                            if (!f.isItemEqual(is) || !ItemStack.areItemStackTagsEqual(f, is)) return true;
                            else return false;
                        } else {
                            if (f.isItemEqual(is) && ItemStack.areItemStackTagsEqual(f, is)) return true;
                        }
            ...
            boolean ignoreMeta = MiscUtils.getStackTag(filter)
                .getBoolean("ignoreMeta");
            boolean ignoreNBT = MiscUtils.getStackTag(filter)
                .getBoolean("ignoreNBT");
            boolean ignoreOreDict = MiscUtils.getStackTag(filter)
                .getBoolean("ignoreOreDict");
            for (int i = 0; i < filterInventory.getSizeInventory(); ++i) {
                ItemStack f = filterInventory.getStackInSlot(i);
                if (f != null) {
                    if (f.getItem() instanceof ItemFilter) {
                        ...
                        if (filter.getItemDamage() == 1) {
                            if (oreDictionaryCompare(is, f) || ignoreOreDict) {
                                ...
                                if (ItemStack.areItemStacksEqual(is, f)
                                    || (is.getItem() == f.getItem() && ignoreMeta)) {
                                    if (ItemStack.areItemStackTagsEqual(f, is) || ignoreNBT) {
                                        return true;
                                    }
                                }
    ...
    public static void actionsTick() {
        if (!actions.isEmpty()) for (int i = 0; i < actions.size(); ++i) {
    ...
    public static void addScheduledAction(ScheduledServerAction ssa) {
        if (FMLCommonHandler.instance()
            ...
            Notifier.notifyCustomMod(
                "EssentialCraft",
                "[WARNING][SEVERE]Trying to add a scheduled server action not on server side, aborting!");
    ...
    public static void requestScheduledTileSync(TileEntity tile, EntityPlayer requester) {
        Side s = FMLCommonHandler.instance()
            .getEffectiveSide();
        if (s == Side.CLIENT) {
            if (tile.getWorldObj() == null || tile.getWorldObj().provider == null) return;
    ...
    public static GameProfile EC3FakePlayerProfile = new GameProfile(
        UUID.fromString("5cd89d0b-e9ba-0000-89f4-b5dbb05963da"),
        "[EC3]");
}
