package ec3.utils.dummycore.ASM;
...
import ec3.utils.dummycore.config.CustomTXTConfig;
...
public class DCASMManager implements IClassTransformer {
    ...
    public byte[] transform(String name, String transformedName, byte[] basicClass) {
        ...
        if (basicClass != null) // If the class we are loading exists
        {
            if (name.equals(FMLClientHandlerName))
                ...
                if (name.endsWith(NetHandlerPlayClientName) || name.endsWith(NotchNetHandlerPlayClientName)
                    || name.endsWith(ObfNetHandlerPlayClientName)) {
                    ...
                    classReader.accept(classNode, 0); // Giving our helper a parsed list of commands without any code
                                                      // modifications.
                    if (classNode.invisibleAnnotations != null && classNode.invisibleAnnotations.size() > 0) // If we
                                                                                                             // have
                                                                                                             // some
                                                                                                             // invisible
                                                                                                             // annotations
                                                                                                             ...
                                                                                                             // class.
                    {
                        ...
                        for (int i = 0; i < classNode.invisibleAnnotations.size(); ++i) // Looking through all
                        ...
                        // if (checkClass) return handleClass(name, transformedName, basicClass, classNode,
                        // classReader); // If
                        // // class
                        // // requires
                        // // inspection
    ...
    public byte[] handleNetHandlerPlayClientClass(String name, String transformedName, byte[] basicClass) {
        int obfuscationType = name.endsWith(NetHandlerPlayClientName) ? 0
            : name.endsWith(ObfNetHandlerPlayClientName) ? 1 : 2;
        String S0FPacketSpawnMob = obfuscationType == 0 || obfuscationType == 1
            ? "net/minecraft/network/play/server/S0FPacketSpawnMob"
            : "fz";
        String EntityLivingBase = obfuscationType == 0 ? "net/minecraft/entity/EntityLivingBase"
            : obfuscationType == 1 ? "net/minecraft/entity/EntityLivingBase" : "sv";
        if (!CustomTXTConfig.init) CustomTXTConfig.createCFG();
        if (CustomTXTConfig.mappings.containsKey("fixS0FSpawnMobPacketCrash")
            && CustomTXTConfig.mappings.get("fixS0FSpawnMobPacketCrash")
                .contains("true")) {
            ...
            try {
                ...
                cr.accept(cn, ClassReader.EXPAND_FRAMES); // <---- giving cn the instruction list. 0 is for flags, we
                                                          // want none.

                List<MethodNode> methods = cn.methods;
                for (int i = 0; i < methods.size(); ++i) {
                    MethodNode method = methods.get(i);
                    if (method.name.equals(NetHandlerPlayClient_MethodHandleSpawnMobName)
                        || method.name.equals(ObfNetHandlerPlayClient_MethodHandleSpawnMobName)
                        || method.name.equals(NotchNetHandlerPlayClient_MethodHandleSpawnMobName)) {
                        if (method.desc.equals(NetHandlerPlayClient_MethodHandleSpawnMobDesc)
                            || method.desc.equals(ObfNetHandlerPlayClient_MethodHandleSpawnMobDesc)
                            || method.desc.equals(NotchNetHandlerPlayClient_MethodHandleSpawnMobDesc)) {
                            ...
                            for (int j = 0; j < method.instructions.size(); ++j) {
                                ...
                                if (ain instanceof TypeInsnNode && ain.getOpcode() == Opcodes.CHECKCAST) {
                                    if (method.instructions.size() > j + 1) {
                                        ...
                                        if (ain1 instanceof VarInsnNode && ain1.getOpcode() == Opcodes.ASTORE
                                            && ain.getType() == Type.OBJECT) {
                                            insertAfter = ain1;
                                            break;
                                        }
                                    }
                            ...
                            if (insertAfter != null) {
                                ...
                                inst.add(
                                    new LdcInsnNode(
                                        "Handled an attempt to spawn a null mob on client, aborting! This might break everything completely though."));
                                inst.add(
                                    new MethodInsnNode(
                                        Opcodes.INVOKESTATIC,
                                        "DummyCore/Utils/Notifier",
                                        "notifyErrorCustomMod",
                                ...
                                inst.add(
                                    new FrameNode(
                                        Opcodes.F_FULL,
                                        8,
                                        new Object[] { name.replace('.', '/'), S0FPacketSpawnMob, Opcodes.DOUBLE,
                                            Opcodes.DOUBLE, Opcodes.DOUBLE, Opcodes.FLOAT, Opcodes.FLOAT,
                                            EntityLivingBase },
                                        0,
                                        new Object[] {}));
                ...
                ClassWriter cw = new ClassWriter(ClassWriter.COMPUTE_MAXS); // <---- Creating the way we are writing
                                                                            // Instructions into bytecode. COMPUTE_MAXS
                                                                            // will automatically calculate STACK size
                                                                            // and LOCAL count for each method.
                ...
                LogManager.getLogger()
                    .error("[DummyCore][ASM]Failed to modify " + NetHandlerPlayClientName, e);
                return defaultBytes;
            }
        }
    ...
    public byte[] handleForgeClientHandlerClass(String name, String transformedName, byte[] basicClass) {
        if (!CustomTXTConfig.init) CustomTXTConfig.createCFG();
        if (!CustomTXTConfig.mappings.containsKey("insertDCCallInTextureLoader")
            || !CustomTXTConfig.mappings.get("insertDCCallInTextureLoader")
                .equals("true"))
            return basicClass;
        ...
        try {
            ClassNode cn = new ClassNode(); // <---- Actually creates the EMPTY class
            ClassReader cr = new ClassReader(basicClass); // <---- Bytecode -> Instructions
            cr.accept(cn, ClassReader.EXPAND_FRAMES); // <---- giving cn the instruction list. 0 is for flags, we want
                                                      // none.

            List<MethodNode> methods = cn.methods;
            for (int i = 0; i < methods.size(); ++i) {
                MethodNode method = methods.get(i);
                if (method.name.equals(logMissingTextureErrorsName)
                    && method.desc.equals(logMissingTextureErrorsDesc)) {
                    method.instructions.insert(new LabelNode());
                    method.instructions.insert(
                        new MethodInsnNode(
                            Opcodes.INVOKESTATIC,
                            "DummyCore/Core/CoreInitialiser",
                            "fmlLogMissingTextures",
                            "()V",
                            false));
                }
            }
            ...
            ClassWriter cw = new ClassWriter(ClassWriter.COMPUTE_MAXS); // <---- Creating the way we are writing
                                                                        // Instructions into bytecode. COMPUTE_MAXS will
                                                                        // automatically calculate STACK size and LOCAL
            ...
            LogManager.getLogger()
                .error("[DummyCore][ASM]Failed to modify " + FMLClientHandlerName, e);
            return defaultBytes;
        }
    }
    ...
    // "DummyCoreASM",
    // "Class " + name
    // + " has requested a DummyCore ASM check via DummyCore/Utils/DCASMCheck annotation. Examining...");
    ...
    // "DummyCoreASM",
    // "Class " + name
    // + " has requested a DummyCore ASM check on it's implementations via DummyCore/Utils/ExistanceCheck annotation.
    // Examining...");
    ...
    // // class
    // {
    // if (cn.interfaces.get(j)
    // .equalsIgnoreCase(checkedClss[i].replace('.', '/'))) // If thi is the one we are looking for
    // {
    // Notifier.notifyCustomMod(
    // "DummyCoreASM",
    // "Class " + name
