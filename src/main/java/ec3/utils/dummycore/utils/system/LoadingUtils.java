...
public class LoadingUtils {
    ...
    public static void makeACrash(String message, Class<?> thrower, Throwable t, boolean shutdown) {
        ...
        for (ModContainer mCon : tryDetermineModOffendorsFromClasses(tryDetermineOffendorsFromTrace(t)))
            Notifier.notify(
                ...
                Notifier.error,
                "[DummyCoreLoader]",
                "Likely involved mod: " + mCon.getName()
                    + "[modid:"
                    + mCon.getModId()
                    + ",modClasspath:"
                    + (mCon.getMod() != null ? mCon.getMod()
                        .getClass() : "NULL{mod has no mod instance?}")
                    + "]");

        if (shutdown) FMLCommonHandler.instance()
            .getSidedDelegate()
            .haltGame(message, t);
    }
    ...
    public static Class<?>[] tryDetermineOffendorsFromTrace(Throwable t) {
        ...
        for (StackTraceElement ste : elements) {
            ...
            try {
                ...
                if (offendor != null) offendors[i] = offendor;
    ...
    public static ModContainer[] tryDetermineModOffendorsFromClasses(Class<?>... detectedInTrace) {
        ...
        for (Class<?> clazz : detectedInTrace) {
            for (int i = 0; i < Loader.instance()
                .getActiveModList()
                .size(); ++i) {
                ModContainer mc = Loader.instance()
                    .getActiveModList()
                    .get(i);
                if (mc.getMod() != null && mc.getMod()
                    .getClass()
                    .equals(clazz)) {
