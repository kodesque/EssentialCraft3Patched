...
// "[DCVersionChecker]Catched an attempt to add a version check request for a non registered mod!");
// }
//
// public static void dispatchModChecks() {
// for (int i = 0; i < vCheckRequesters.size(); ++i) requestModVCheck(vCheckRequesters.get(i));
// }
//
// public static void requestModVCheck(DCMod mod) {
// if (vCheckRequesters.contains(mod)) new ThreadURICheck(mod, modsURIs.get(mod)).start();
// }
//
...
// for (int i = 0; i < versions.length; ++i) versions[i] = removeButNumbers(versions[i]);
//
// for (int i = 0; i < cVersions.length; ++i) cVersions[i] = removeButNumbers(cVersions[i]);
//
// for (int i = 0; i < 4; ++i) {
// int ii = i;
// if (ii == 0) ii = 2;
...
// int current = Integer.parseInt(pcurrentVersion);
// int latest = Integer.parseInt(pcheckedVersion);
// if (current < latest) tryNotifySimpleOutdatedMod(mod, checkedVersion);
// }
...
// EntityPlayer player = CoreInitializer.proxy.getClientPlayer();
// if (player != null) {
// String steppedString = "";
// switch (step) {
// case 0: {
// steppedString = String.format(
// "%s is horribly outdated(Your version is %s, latest is %s) has bugs and no longer supported! Update as soon as
// possible!",
// mod.ufName,
// mod.version,
// latest);
...
// case 1: {
// steppedString = String.format(
// "%s is outdated(Your version is %s, latest is %s)! Consider updating!",
// mod.ufName,
// mod.version,
// latest);
// break;
...
// case 2: {
// steppedString = String.format(
// "%s is available for another version of the game(Your version is %s, latest is %s)!",
// mod.ufName,
// mod.version,
// latest);
// break;
...
// case 3: {
// steppedString = String.format(
// "%s has a new version available(Your version is %s, latest is %s)!",
// mod.ufName,
// mod.version,
// latest);
// break;
...
// new ChatComponentText(steppedString).setChatStyle(new ChatStyle().setColor(EnumChatFormatting.RED)));
// }
// }
//
...
// mod.ufName,
// mod.version,
// latest)).setChatStyle(new ChatStyle().setColor(EnumChatFormatting.RED)));
// }
//
...
// for (char c : chars) if (Character.isDigit(c)) result += c;
//
// return result;
// }
//
