...
public class PlayerTickHandler {
    ...
    public void client_manageBook(EntityPlayer e) {
        if (Keyboard.isKeyDown(Keyboard.KEY_R) && !isRKeyPressed) {
            ...
            if (currentPlayerItem != null && currentPlayerItem.getItem() == ECItems.research_book) {
                boolean foundItem = false;
                Vec3 playerLookVec = e.getLookVec();
                Vec3 itemSearchVec = Vec3
                    .createVectorHelper(playerLookVec.xCoord, playerLookVec.yCoord, playerLookVec.zCoord);
                // Searching for EntityItem
                s: for (int o = 0; o < 4; ++o) {
                    itemSearchVec.xCoord *= o + 1;
                    itemSearchVec.yCoord *= o + 1;
                    itemSearchVec.zCoord *= o + 1;
                    List<EntityItem> lst = e.worldObj.getEntitiesWithinAABB(
                        EntityItem.class,
                        AxisAlignedBB.getBoundingBox(
                            e.posX + itemSearchVec.xCoord - 0.5D,
                            e.posY + itemSearchVec.yCoord - 0.5D,
                            e.posZ + itemSearchVec.zCoord - 0.5D,
                            e.posX + itemSearchVec.xCoord + 0.5D,
                            e.posY + itemSearchVec.yCoord + 0.5D,
                            e.posZ + itemSearchVec.zCoord + 0.5D));
                    if (!lst.isEmpty()) {
                        for (EntityItem itm : lst) {
                            if (itm != null && !itm.isDead) {
                                ItemStack heldIS = itm.getEntityItem()
                                    .copy();
                                if (ApiCore.findDiscoveryByIS(heldIS) != null) {
                                    ...
                                    if (GuiResearchBook.currentCategory == null)
                                        GuiResearchBook.currentCategory = ECResearch.basic;
                ...
                if (!foundItem) {
                    ...
                    s: for (int o = 0; o < 4; ++o) {
                        ...
                        Block blk = e.worldObj.getBlock(
                            MathHelper.floor_double(blockSearchVec.xCoord + e.posX),
                            MathHelper.floor_double(blockSearchVec.yCoord + e.posY),
                            MathHelper.floor_double(blockSearchVec.zCoord + e.posZ));
                        if (blk != null && blk != Blocks.air) {
                            ItemStack heldIS = new ItemStack(
                                blk,
                                1,
                                e.worldObj.getBlockMetadata(
                                    MathHelper.floor_double(blockSearchVec.xCoord + e.posX),
                                    MathHelper.floor_double(blockSearchVec.yCoord + e.posY),
                                    MathHelper.floor_double(blockSearchVec.zCoord + e.posZ)));
                            if (ApiCore.findDiscoveryByIS(heldIS) != null) {
    ...
    public void client_manageWorldEvents(EntityPlayer e) {
        if (e.dimension == Config.dimensionID) {
            if (e.worldObj.provider.dimensionId == Config.dimensionID) {
                ((WorldProviderHoanna) (e.worldObj.provider)).generateLightBrightnessTable();
                if (ECUtils.isEventActive("ec3.event.darkness")) {
                    if (e.worldObj.rand.nextFloat() < 0.01F) e.worldObj.playSound(
                        e.posX,
                        e.posY,
                        e.posZ,
                        "ambient.cave.cave",
                        1,
                        e.worldObj.rand.nextFloat() * 2,
                        true);
                    if (e.worldObj.rand.nextFloat() < 0.001F) {
                        String[] sound = { "mob.zombie.death", "mob.zombie.say", "mob.blaze.death", "mob.skeleton.step",
                            "mob.endermen.stare", "mob.spider.step", "mob.spider.death", "mob.spider.say",
                            "mob.creeper.death" };

                        e.worldObj.playSound(
                            e.posX + MathUtils.randomDouble(e.worldObj.rand) * 16,
                            e.posY,
                            e.posZ + MathUtils.randomDouble(e.worldObj.rand) * 16,
                            sound[e.worldObj.rand.nextInt(sound.length)],
                            1,
                            0.01F,
                            true);
                    }
                ...
                if (b != null) {
                    for (int i = 0; i < b.getSizeInventory(); ++i) {
                        ...
                        if (is != null && is.getItem() != null
                            ...
                            && is.getItemDamage() == 19) ignoreEarthquake = true;
                    }
                }
            ...
            if (f != null) {
                ...
                try {
                    ...
                    try {
                        ...
                        ECUtils.getData(e)
                            .writeToNBTTagCompound(tag);
                        CompressedStreamTools.writeCompressed(tag, oStream);
                    } catch (Exception Ex) {
                        FMLCommonHandler.instance()
                            .raiseException(
                                Ex,
                                "EssentialCraft3 Encountered an exception whlist saving playerdata NBT of player "
                                    + e.getCommandSenderName()
                                    + "!Report the error to the forum - http://www.minecraftforum.net/forums/mapping-and-modding/minecraft-mods/2286105",
                                false);
                    } finally {
                        oStream.close();
                    }
                    ...
                    FMLCommonHandler.instance()
                        .raiseException(
                            Exx,
                            "EssentialCraft3 Encountered an exception whlist wrighting playerdata file of player "
                                + e.getCommandSenderName()
                                + "! Make sure, that the file is not being accessed by other applications and is not threated as a virus by your anti-virus software! Also make sure, that you have some harddrive space. If everything above is correct - report the error to the forum - http://www.minecraftforum.net/forums/mapping-and-modding/minecraft-mods/2286105",
                            true);
                }
            }
        }
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            FMLLog.log(
                                Level.WARN,
                                "[EC3]Player data for player " + e.getCommandSenderName()
                                    + " could not be read. If it is the first time of the player to log in - it is fine. Otherwise, report the error to the author!");
                        }

                        if (tag != null) ECUtils.readOrCreatePlayerData(e, tag);
                        else ECUtils.createPlayerData(e);
                    } catch (Exception Ex) {
                        FMLCommonHandler.instance()
                            .raiseException(
                                Ex,
                                "EssentialCraft3 Encountered an exception whlist reading playerdata NBT of player "
                                    + e.getCommandSenderName()
                                    + "! It is totally fine if this is your first time opening the save. If it is not - report the error to the forum - http://www.minecraftforum.net/forums/mapping-and-modding/minecraft-mods/2286105",
                                false);
                    ...
                    FMLCommonHandler.instance()
                        .raiseException(
                            Exx,
                            "EssentialCraft3 Encountered an exception whlist opening playerdata file of player "
                                + e.getCommandSenderName()
                                + "! Make sure, that the file is not being accessed by other applications and is not threated as a virus by your anti-virus software! Also make sure, that you have some harddrive space. If everything above is correct - report the error to the forum - http://www.minecraftforum.net/forums/mapping-and-modding/minecraft-mods/2286105",
                            true);
                }
    ...
    public void tickEvent(PlayerTickEvent event) {
        if (event.phase == Phase.END) {
            ...
            if (!isFlightAllowed.containsKey(e)) isFlightAllowed.put(e, false);

            // Step assistant, should 100% work with TC's system, not sure about Vazkii's one... If she calls it once
            // per tick, it is fine. If it is only onEquiped(), then his baubles need to be re-equipped in order to
            // work.
            if (!isWearingBoots.containsKey(e)) isWearingBoots.put(e, false);

            if (e.inventory.armorInventory[0] != null
                && e.inventory.armorInventory[0].getItem() instanceof ItemComputerArmor) {
                e.fallDistance = 0;
            }

            if (e.inventory.armorInventory[3] != null
                && e.inventory.armorInventory[3].getItem() instanceof ItemComputerArmor) {
                e.setAir(300);
                e.getFoodStats()
                    .addStats(1, 1);
            }

            if (e.inventory.armorInventory[1] != null
                && e.inventory.armorInventory[1].getItem() instanceof ItemComputerArmor) {
            ...
            if (e.inventory.armorInventory[0] != null
                && (e.inventory.armorInventory[0].getItem() instanceof ItemGenericArmor
                    || e.inventory.armorInventory[0].getItem() instanceof ItemComputerArmor
                    || e.inventory.armorInventory[0].getItem() == ECItems.magicArmorItems[7])
                && (!isWearingBoots.get(e) || e.stepHeight < 1F)) {
            ...
            if ((e.inventory.armorInventory[0] == null
                || !(e.inventory.armorInventory[0].getItem() instanceof ItemGenericArmor
                    || e.inventory.armorInventory[0].getItem() instanceof ItemComputerArmor
                    || e.inventory.armorInventory[0].getItem() == ECItems.magicArmorItems[7]))
                && isWearingBoots.get(e)) {
                isWearingBoots.put(e, false);
                e.stepHeight = 0.5F;
            }

            if (BaublesApi.getBaubles(e) != null) {
                ItemStack belt = BaublesApi.getBaubles(e)
                    .getStackInSlot(3);
                if (belt != null) {
                        ...
                        if (s == Side.CLIENT) {
                            if (client_flightAllowed) {
                                ...
                                if (e.capabilities.allowFlying && !e.capabilities.isCreativeMode) {
                                    ...
                                    if (e.capabilities.isFlying) e.capabilities.isFlying = false;
            ...
            if (e.ticksExisted % 20 != 0) return;

            // Server overview
            if (!e.worldObj.isRemote) {
                manageSync(e);
                manageWorldSync(e);
                WindUtils.playerTick(e);
                RadiationUtils.playerTick(e);
                // System.out.println(ECUtils.getData(e).damage);
                if (e.ticksExisted % 200 == 0) ECUtils.requestSync(e);
                if (e.ticksExisted % 1200 == 0) {
                    ...
                    if (e.worldObj.rand.nextInt(36000) <= data.getOverhaulDamage()) {
                        ...
                        ArrayList<ICorruptionEffect> costSelected = CorruptionEffectLibrary
                            .findSutableEffects(data.getOverhaulDamage());
                        for (int i = 0; i < costSelected.size(); ++i) {
                            ICorruptionEffect selected = costSelected.get(i);
                            boolean canAdd = selected != null;
                            if (selected != null) {
                                if (!playerEffects.isEmpty()) J: for (int j = 0; j < playerEffects.size(); ++j) {
                                    ICorruptionEffect playerEffect = playerEffects.get(j);
                                    if (playerEffect == null) continue J;
                                ...
                                if (canAdd) possibleEffects.add(selected);
                            }
                        }
                        if (!possibleEffects.isEmpty()) {
                            ICorruptionEffect added = possibleEffects
                                .get(e.worldObj.rand.nextInt(possibleEffects.size()))
                                .copy();
                            data.modifyOverhaulDamage(data.getOverhaulDamage() - added.getStickiness());
                            data.getEffects()
                                .add(added);
                            e.addChatMessage(
                                new ChatComponentText(StatCollector.translateToLocal("ec3.txt.corruption"))
                ...
                List<EntityItem> itemList = wrd.getEntitiesWithinAABB(
                    EntityItem.class,
                    AxisAlignedBB
                        .getBoundingBox(
                            e.posX - 0.5D,
    ...
    public void doGroundItemChecks(EntityItem item) {
        if (item.getEntityItem()
            ...
            Block b = item.worldObj.getBlock(
                MathHelper.floor_double(item.posX),
                MathHelper.floor_double(item.posY) - 1,
                MathHelper.floor_double(item.posZ));
            if (b == Blocks.netherrack) {
                Block b_check_b1 = item.worldObj.getBlock(
                    MathHelper.floor_double(item.posX) + 1,
                    MathHelper.floor_double(item.posY) - 1,
                    MathHelper.floor_double(item.posZ));
                Block b_check_b2 = item.worldObj.getBlock(
                    MathHelper.floor_double(item.posX) - 1,
                    MathHelper.floor_double(item.posY) - 1,
                    MathHelper.floor_double(item.posZ));
                Block b_check_b3 = item.worldObj.getBlock(
                    MathHelper.floor_double(item.posX),
                    MathHelper.floor_double(item.posY) - 1,
                    MathHelper.floor_double(item.posZ) + 1);
                Block b_check_b4 = item.worldObj.getBlock(
                    MathHelper.floor_double(item.posX),
                    MathHelper.floor_double(item.posY) - 1,
                    MathHelper.floor_double(item.posZ) - 1);
                Block b_check_air = item.worldObj
                    .getBlock(MathHelper.floor_double(item.posX), (int) item.posY, MathHelper.floor_double(item.posZ));
                if (b_check_air.isAir(
                    item.worldObj,
                    MathHelper.floor_double(item.posX),
                    (int) item.posY,
                    MathHelper.floor_double(item.posZ)) && b_check_b1.getMaterial() == Material.lava
                    && b_check_b4.getMaterial() == Material.lava
                    && b_check_b2.getMaterial() == Material.lava
                    && b_check_b3.getMaterial() == Material.lava) {
                    if (item.getEntityItem().stackSize == 1) item.lifespan = 0;
                    else {
            ...
            .getItem() == Items.clay_ball) {
                ...
                if (b == Blocks.ice) {
                    ...
                    Block b_check_b4 = item.worldObj.getBlock(
                        ...
                        MathHelper.floor_double(item.posY) - 1,
                        MathHelper.floor_double(item.posZ) - 1);
                    Block b_check_air = item.worldObj.getBlock(
                        MathHelper.floor_double(item.posX),
                        (int) item.posY,
                        MathHelper.floor_double(item.posZ));
                    if (b_check_air.isAir(
                        ...
                        MathHelper.floor_double(item.posX),
                        (int) item.posY,
                        MathHelper.floor_double(item.posZ)) && b_check_b1.getMaterial() == Material.water
                ...
                .getItem() == Items.slime_ball) {
                    ...
                    if (b == Blocks.grass) {
                        ...
                        if (b_check_air.isAir(
                            ...
                            MathHelper.floor_double(item.posX),
                            (int) item.posY,
                            MathHelper.floor_double(item.posZ)) && b_check_b1 == Blocks.mossy_cobblestone
                            && b_check_b4 == Blocks.mossy_cobblestone
                            && b_check_b2 == Blocks.mossy_cobblestone
                            && b_check_b3 == Blocks.mossy_cobblestone) {
                            if (item.getEntityItem().stackSize == 1) item.lifespan = 0;
                            else {
                                item.getEntityItem().stackSize -= 1;
                            }
                    ...
                    .getItem() == Items.gunpowder) {
                        ...
                        if (b == Blocks.quartz_block) {
                            ...
                            if (b_check_air.isAir(
                                ...
                                MathHelper.floor_double(item.posX),
                                (int) item.posY,
                                MathHelper.floor_double(item.posZ)) && b_check_b1 == Blocks.sand
                                && b_check_b4 == Blocks.sand
                                && b_check_b2 == Blocks.sand
                                && b_check_b3 == Blocks.sand) {
                                if (item.getEntityItem().stackSize == 1) item.lifespan = 0;
                                else {
                                    item.getEntityItem().stackSize -= 1;
                                }
                                ...
                                if (b == Blocks.emerald_block) {
                                    ...
                                    if (b_check_air.isAir(
                                        ...
                                        if (item.worldObj.rand.nextFloat() <= 0.5F) {
                                            EntityItem soulStone = new EntityItem(
                                                item.worldObj,
                                                (float) item.posX,
                                                (float) item.posY,
                                                (float) item.posZ,
