package ec3.client.render.handlers;
...
public class RenderHandlerEC {
    ...
    public static final ResourceLocation whitebox = new ResourceLocation(
        "essentialcraft",
        "textures/special/whitebox.png");
    ...
    public static final IModelCustom board = AdvancedModelLoader
        .loadModel(new ResourceLocation("essentialcraft", "textures/special/models/board.obj"));
    public static final ResourceLocation boardTextures = new ResourceLocation(
        "essentialcraft",
        "textures/special/models/board.png");
    ...
    public void renderParadox() {
        ...
        if (paradoxID == 0) {
            if (currentParadoxTicks == 199) {
                mc.thePlayer.worldObj.playSound(
                    mc.thePlayer.posX,
                    mc.thePlayer.posY,
                    mc.thePlayer.posZ,
                    "ambient.cave.cave",
            ...
            if (currentParadoxTicks >= 10) for (int i = 0; i < 20; ++i) {
                mc.theWorld.spawnParticle(
                    "reddust",
                    mc.thePlayer.posX + MathUtils.randomDouble(mc.theWorld.rand) * 16,
                    mc.thePlayer.posY + MathUtils.randomDouble(mc.theWorld.rand) * 16,
                    mc.thePlayer.posZ + MathUtils.randomDouble(mc.theWorld.rand) * 16,
                    -1,
                    0,
                    0);
            }
        ...
        if (paradoxID == 2) {
            ...
            if (currentParadoxTicks < 190 && currentParadoxTicks > 10) {
                if (explosion == null) {
                    explosion = new Coord3D(
                        mc.thePlayer.posX + MathUtils.randomDouble(mc.theWorld.rand) * 32,
                        mc.thePlayer.posY + 32,
                        mc.thePlayer.posZ + MathUtils.randomDouble(mc.theWorld.rand) * 32);
    ...
    public static IInventory getInventoryFromContainer(GuiContainer gc) {
        for (int i = 0; i < gc.inventorySlots.inventorySlots.size(); ++i) {
            ...
            if (slt != null) return slt.inventory;
        }
        return null;
    }
        ...
        if (stk != null) {
            ...
            if (item instanceof ItemGenericArmor) {
                ...
                Minecraft.getMinecraft().renderEngine
                    .bindTexture(RenderBiped.getArmorResource(event.entityPlayer, stk, slot, null));
                RenderPlayer rp = (RenderPlayer) RenderManager.instance
                    .getEntityRenderObject(Minecraft.getMinecraft().thePlayer);
                ...
                modelbiped = net.minecraftforge.client.ForgeHooksClient
                    .getArmorModel(event.entityPlayer, stk, slot, modelbiped);
    ...
    public void renderPlayerSpecials(RenderPlayerEvent.Specials.Post event) {
        ...
        if (BaublesApi.getBaubles(p) != null) {
            if (BaublesApi.getBaubles(p)
                .getStackInSlot(3) == null
                || !(BaublesApi.getBaubles(p)
                    .getStackInSlot(3)
                    .getItem() instanceof ItemComputerBoard))
                return;

            if (!p.capabilities.isFlying) return;
        ...
        if (is != null) {
            if (is.getItem() instanceof ItemOrbitalRemote) {
                ...
                double d1 = player.prevPosY + (player.posY - player.prevPosY) * (double) f
                    + (double) (player.getEyeHeight() - player.getDefaultEyeHeight()); // isRemote check to revert
                                                                                       // changes to ray trace position
                                                                                       // due to adding the eye height
                                                                                       // clientside and player yOffset
                                                                                       // differences
                ...
                if (mop != null && mop.typeOfHit == MovingObjectType.BLOCK) {
                    ...
                    GL11.glTranslated(
                        (double) mop.blockX - TileEntityRendererDispatcher.staticPlayerX,
                        (double) mop.blockY - TileEntityRendererDispatcher.staticPlayerY,
                        (double) mop.blockZ - TileEntityRendererDispatcher.staticPlayerZ);
                    if (f1 > 0.0F) {
    ...
    public void renderBlockOverlays(RenderWorldLastEvent evt) {
        ...
        if (is != null) {
            if (is.getTagCompound() != null && is.getItem() instanceof ItemBoundGem
                && is.getTagCompound()
                    .hasKey("pos")
                && is.getTagCompound()
                    .getInteger("dim") == p.dimension) {
                ...
                if (!p.worldObj.blockExists((int) mx, (int) my, (int) mz)) return;

                AxisAlignedBB aabb = p.worldObj.getBlock((int) mx, (int) my, (int) mz)
                    .getSelectedBoundingBoxFromPool(p.worldObj, (int) mx, (int) my, (int) mz);
            ...
            if (is.getItem() instanceof ItemMagicalBuilder) {
                ...
                if (builder.hasFirstPoint(is) && !builder.hasSecondPoint(is)) {
                    ...
                    AxisAlignedBB aabb = AxisAlignedBB.getBoundingBox(
                        ...
                        c1.y < c.y ? c.y + 1 : c.y,
                        c1.z < c.z ? c.z + 1 : c.z,
                        c1.x < c.x ? c1.x : c1.x + 1,
                        c1.y < c.y ? c1.y : c1.y + 1,
                        c1.z < c.z ? c1.z : c1.z + 1);
                        ...
                        for (int i = 0; i < gc.inventorySlots.inventorySlots.size(); ++i) {
                            ...
                            if ((slt.inventory instanceof TileEntity) || (slt.inventory instanceof InventoryBasic)) {
                                ...
                                Minecraft.getMinecraft().fontRenderer.drawString(
                                    "" + slt.slotNumber,
                                    (k + slt.xDisplayPosition) * 2,
                                    (l + slt.yDisplayPosition) * 2,
                                    0x000000);
                                GL11.glPopMatrix();
                                if (slt.inventory instanceof ISidedInventory) {
                                    if (RenderHandlerEC.slotsTable.containsKey(inv)) {
                                        Hashtable<Integer, List<ForgeDirection>> accessibleSlots = RenderHandlerEC.slotsTable
                                            .get(inv);
    ...
    public void onPlayerTick(TickEvent.PlayerTickEvent event) {
        if (event.phase == Phase.START) {
            if (ItemInventoryGem.clickTicks > 0) --ItemInventoryGem.clickTicks;
            if (ItemInventoryGem.clickTicks == 0) ItemInventoryGem.currentlyClicked = null;
        }
        for (int i = 0; i < ClientProxy.playingMusic.size(); ++i) {
            ISound snd = ClientProxy.playingMusic.get(i)
                .getSecond();
            if (!Minecraft.getMinecraft()
                .getSoundHandler()
                .isSoundPlaying(snd)) ClientProxy.playingMusic.remove(i);
        }
            ...
            if (event.player instanceof EntityClientPlayerMP) {
                ...
                if (ECPotions.paradox != null && mc.thePlayer.getActivePotionEffect(ECPotions.paradox) != null) {
                    int duration = mc.thePlayer.getActivePotionEffect(ECPotions.paradox)
                        .getDuration();
                    if (duration > 100) {
                        if (duration % 100 == 0) mc.theWorld.playSound(
                            mc.thePlayer.posX,
                            mc.thePlayer.posY,
                            mc.thePlayer.posZ,
                            "essentialcraft:sound.heartbeat",
                            100,
                            1,
                            true);
                        if (currentParadoxTicks > 0) --currentParadoxTicks;
                ...
                if (renderPartialTicksCheck > 0) {
                    ...
                    if (renderPartialTicksCheck <= 0) Minecraft.getMinecraft().theWorld
                        .playSound(player.posX, player.posY, player.posZ, "random.orb", 1F, 2F, false);
                }
                ...
                if (isNightVisionActive && (player.inventory.armorInventory[3] == null
                    || !(player.inventory.armorInventory[3].getItem() instanceof ItemComputerArmor))) {
                ...
                if (ClientProxy.kbArmorBoost.getIsKeyPressed() && !isSprintKeyDown) {
                    ...
                    if (ItemComputerArmor.class.cast(ECItems.computer_helmet)
                        .hasFullset(player) && renderPartialTicksCheck <= 0) {
                        Vec3 lookVec = player.getLookVec();
                        player.motionX += lookVec.xCoord * 3;
                        player.motionY += lookVec.yCoord * 3;
                        player.motionZ += lookVec.zCoord * 3;
                        for (int i = 0; i < 10; ++i) player.worldObj.playSound(
                            player.posX,
                            player.posY,
                            player.posZ,
                            "fireworks.largeBlast",
                            1,
                            0.01F + player.worldObj.rand.nextFloat(),
                            false);
                ...
                if (!player.capabilities.isFlying && player.isInWater()
                    && ItemComputerArmor.class.cast(ECItems.computer_helmet)
                        .hasFullset(player)
                    && Minecraft.getMinecraft().gameSettings.keyBindForward.getIsKeyPressed()) {
                    player.motionX *= 1.2D;
                    if (player.motionY > 0) player.motionY *= 1.2D;
                    player.motionZ *= 1.2D;
                    ...
                    for (int i = 0; i < 10; ++i) player.worldObj.spawnParticle(
                        "bubble",
                        d8,
                        player.posY - 0.125D,
                        d9,
    ...
    public void pre(RenderGameOverlayEvent.Pre event) {
        try {
            ...
            if (event.type == ElementType.CROSSHAIRS) {
                if (ItemComputerArmor.class.cast(ECItems.computer_helmet)
                    .hasFullset(Minecraft.getMinecraft().thePlayer)) {
                    ...
                    if (renderTime > 20) renderTime = 40 - renderTime;
                    ...
                    MiscUtils
                        .drawTexture_Items((int) k - 8, (int) h - 8, Items.fireworks.getIconFromDamage(0), 16, 16, 100);
                ...
                if (Minecraft.getMinecraft().thePlayer.inventory.armorInventory[3] != null
                    && Minecraft.getMinecraft().thePlayer.inventory.armorInventory[3]
                        .getItem() instanceof ItemComputerArmor) {
                    ScaledResolution res = new ScaledResolution(mc, mc.displayWidth, mc.displayHeight);
                    float r = 10;
                    ...
                    MiscUtils.drawTexture_Items(
                        (int) k - 8,
                        (int) h - 8,
                        ECItems.computer_helmet.getIconFromDamage(0),
                        16,
                        16,
                        100);
            ...
            if (event.type == ElementType.HOTBAR) {
                ...
                MovingObjectPosition target = p
                    .rayTrace(mc.playerController.getBlockReachDistance(), event.partialTicks);
                        ...
                        if (rune != null) {
                            if (p.getCurrentEquippedItem() != null) {
                                ...
                                if (rec != null) {
                                    ...
                                    if (energy < energyReq && !creative) color = 0xff0000;
                                    else color = 0x00ff66;

                                    ScaledResolution res = new ScaledResolution(mc, mc.displayWidth, mc.displayHeight);
                                    String displayed = energyReq + " ESPE";
                                    if (creative) displayed += " [Creative]";
                                    Minecraft.getMinecraft().fontRenderer.drawString(
                                        displayed,
                                        res.getScaledWidth() / 2 - displayed.length() * 3,
                                        res.getScaledHeight() / 2 + 5,
                                        color);
                                }
                            }
                        }
    ...
    public void post(RenderGameOverlayEvent.Post event) {
        try {
            if (event.type == ElementType.ALL) {
                ...
                if (mc.thePlayer.getActivePotionEffect(ECPotions.chaosInfluence) != null) {
                    GL11.glDisable(GL11.GL_DEPTH_TEST);
                    GL11.glDepthMask(false);
                    GL11.glEnable(GL11.GL_BLEND);
                    OpenGlHelper.glBlendFunc(770, 771, 1, 0);
                    GL11.glColor4f(1.0F, 1.0F, 1.0F, 0.3F);
                    IIcon iicon = ECBlocks.lightCorruption[0].getIcon(0, 7);
                ...
                if (mc.thePlayer.getActivePotionEffect(ECPotions.frozenMind) != null) {
                    GL11.glDisable(GL11.GL_DEPTH_TEST);
                    GL11.glDepthMask(false);
                    GL11.glEnable(GL11.GL_BLEND);
                    OpenGlHelper.glBlendFunc(770, 771, 1, 0);
                    GL11.glColor4f(1.0F, 1.0F, 1.0F, 0.3F);
                    IIcon iicon = ECBlocks.lightCorruption[1].getIcon(0, 7);
                    mc.getTextureManager()
                        .bindTexture(TextureMap.locationBlocksTexture);
                    float f1 = iicon.getMinU();
                    float f2 = iicon.getMinV();
                ...
                if (mc.thePlayer.getActivePotionEffect(ECPotions.paradox) != null) {
                    /*
    ...
    public static void renderImage(ResourceLocation image, int scaledResX, int scaledResY, float opacity, float r,
        float g, float b) {
        GL11.glDisable(GL11.GL_DEPTH_TEST);
        GL11.glDepthMask(false);
        OpenGlHelper.glBlendFunc(770, 771, 1, 0);
